...
class SmsHelper {
    ...
    public function refund($serverRefCode, $endUserId) {
        ...
        $refCode = getUUID();
        $authKey = config('telenor.TelenorToken');
        $url = "​https://api.comoyo.com/partner/payment/v1/". $endUserId ."/transactions/amount";
    ...
    public function getACR($refid) {
    	$authKey = config('telenor.TelenorToken');
        $url = "http://acr.telenordigital.com/partner/acr/echo?partnerId=miaki&operatorId=TLN-MM&redirect=true&referenceId=" .$refid;
    ...
    public function otpsms($msisdn) {
        $url = 'https://api.comoyo.com/partner/pin';
    ...
    private function postapirequest($url, $data) {
        ...
        $header = array(
            ...
            'Content-Type: application/json',
            'Authorization: Basic ' . config('telenor.TelenorToken')
        );
    ...
    public function confirm_otp($msisdn, $pin) {
        $url = 'https://api.comoyo.com/partner/acrs';
    ...
    public function telenor_mt_sms($endUserId, $msg) {
        $url = 'https://api.comoyo.com/partner/smsmessaging/v2/outbound/tel:+959769867204/requests';
        $data = '
			{  
    ...
    public function payment_with_acr ($endUserId, $amount, $refCode, $consentId) {
    ...
    public function msisdn2acr($msisdn) {
        ...
        //$password='JoUXX7xAKpoYcacnbC7cebTMhrfAzJ3t'; //changed
        $password='yw4ugreyUorWmX3EuqFo9XkR99MP7qLf';
        $URL='https://api.comoyo.com/partner/acrs?msisdn=' . $msisdn;
